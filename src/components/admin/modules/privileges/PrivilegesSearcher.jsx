...
export const PrivilegesSearcher = ({ handleQueryResults, privileges }) => {
  const [inputValue, setInputValue] = useState('');

  useEffect(() => {
    if (inputValue.trim() === '') return handleQueryResults(privileges);

    const filteredPrivileges = privileges.filter(({ nombre }) => {
      return nombre.toLowerCase().includes(inputValue.toLowerCase());
    });

    handleQueryResults(filteredPrivileges);
  }, [inputValue]);
