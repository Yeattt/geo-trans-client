...
export const PermissionsSearcher = ({ handleQueryResults, permissions }) => {
  const [inputValue, setInputValue] = useState('');

  useEffect(() => {
    if (inputValue.trim() === '') return handleQueryResults(permissions);

    const filteredPermissions = permissions.filter(({ nombre }) => {
      return nombre.toLowerCase().includes(inputValue.toLowerCase());
    });

    handleQueryResults(filteredPermissions);
  }, [inputValue]);
