...
export const CompaniesSearcher = ({ handleQueryResults, companies }) => {
  const [inputValue, setInputValue] = useState('');

  useEffect(() => {
    if (inputValue.trim() === '') return handleQueryResults(companies);

    const filteredCompanies = companies.filter(({ nombreEmpresa }) => {
      return nombreEmpresa.toLowerCase().includes(inputValue.toLowerCase());
    });

    handleQueryResults(filteredCompanies);
  }, [inputValue]);
