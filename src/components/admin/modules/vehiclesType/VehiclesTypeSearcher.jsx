...
export const VehiclesTypeSearcher = ({ handleQueryResults, vehiclesType }) => {
  const [inputValue, setInputValue] = useState('');

  useEffect(() => {
    if (inputValue.trim() === '') return handleQueryResults(vehiclesType);

    const filteredVehiclesType = vehiclesType.filter(({ nombre }) => {
      return nombre.toLowerCase().includes(inputValue.toLowerCase());
    });

    handleQueryResults(filteredVehiclesType);
  }, [inputValue]);
